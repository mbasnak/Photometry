...
%% Get the peaks amplitude distribution

figure, set(gcf, 'Position', [800, 800, 800, 800]),

for i = 1:size(peakLocT,2)
    subplot(1,size(peakLocT,2),i)
    peaks = cell2mat(struct2cell(peakLocT(i)));
    histogram(Zdopamine(peaks),8,'FaceColor',rand(1,3));
    ylim([0,150]);
end
suptitle('Dopamine amplitude distribution for different thresholds');
[ax,h1]=suplabel('Dopamine peak value'); 
[ax,h2]=suplabel('Frequency','y');

%get the peaks where the amplitude is bigger than 1 in the zscored original
%data for the different thresholds
for i = 1:size(peakLocT,2)
    peaks = cell2mat(struct2cell(peakLocT(i)));
    A = (Zdopamine(peaks)>1); %extract the peak index for which the Zscored original signal is bigger than 1
    GreaterPeaks{i} = peaks(A);
end
...
%% STA with different peak amplitudes

stimulus = ZD1;
events = zeros(size(dopamine,1),size(peakLocT,2));
windowSize = 500; %number of frames to compute triggered average

for i = 1:size(peakLocT,2)
    GreaterPeaks2 = cell2mat(GreaterPeaks(i));
    events(GreaterPeaks2,i) = 1;
end

[avg1, avg21, nEvs1] = evTrigAvg(events(:,1), stimulus, windowSize);
[avg2, avg22, nEvs2] = evTrigAvg(events(:,2), stimulus, windowSize);
[avg3, avg23, nEvs3] = evTrigAvg(events(:,3), stimulus, windowSize);
[avg4, avg24, nEvs4] = evTrigAvg(events(:,4), stimulus, windowSize);
[avg5, avg25, nEvs5] = evTrigAvg(events(:,5), stimulus, windowSize);
...
ci1 = bootci(5000, @mean, tavg1);
ci2 = bootci(5000, @mean, tavg2);
ci3 = bootci(5000, @mean, tavg3);
ci4 = bootci(5000, @mean, tavg4);
ci5 = bootci(5000, @mean, tavg5);
...
figure, set(gcf, 'Position', [800, 1700, 1700, 800]),
subplot(1,5,1),
plot(timeAxis,mean(avg1,2),'k')
hold on
plot(timeAxis,tci1(:,1),'r--');
plot(timeAxis,tci1(:,2),'r--');
line([0,0],[min(min(tci1))-1E-3, max(max(tci1))+1E-3]);
ylim([min(min(tci1))-1E-3, max(max(tci1))+1E-3]);
xlim([min(timeAxis),max(timeAxis)]);

subplot(1,5,2),
plot(timeAxis,mean(avg2,2),'k')
hold on
plot(timeAxis,tci2(:,1),'r--');
plot(timeAxis,tci2(:,2),'r--');
line([0,0],[min(min(tci2))-1E-3, max(max(tci2))+1E-3]);
ylim([min(min(tci2))-1E-3, max(max(tci2))+1E-3]);
xlim([min(timeAxis),max(timeAxis)]);

subplot(1,5,3),
plot(timeAxis,mean(avg3,2),'k')
hold on
plot(timeAxis,tci3(:,1),'r--');
plot(timeAxis,tci3(:,2),'r--');
line([0,0],[min(min(tci3))-1E-3, max(max(tci3))+1E-3]);
ylim([min(min(tci3))-1E-3, max(max(tci3))+1E-3]);
xlim([min(timeAxis),max(timeAxis)]);

subplot(1,5,4),
plot(timeAxis,mean(avg4,2),'k')
hold on
plot(timeAxis,tci4(:,1),'r--');
plot(timeAxis,tci4(:,2),'r--');
line([0,0],[min(min(tci1))-1E-3, max(max(tci4))+1E-3]);
ylim([min(min(tci4))-1E-3, max(max(tci4))+1E-3]);
xlim([min(timeAxis),max(timeAxis)]);

subplot(1,5,5),
plot(timeAxis,mean(avg5,2),'k')
hold on
plot(timeAxis,tci5(:,1),'r--');
plot(timeAxis,tci5(:,2),'r--');
line([0,0],[min(min(tci5))-1E-3, max(max(tci5))+1E-3]);
ylim([min(min(tci5))-1E-3, max(max(tci5))+1E-3]);
xlim([min(timeAxis),max(timeAxis)]);

suptitle('STA of D1 with greatest dopamine peaks for the different thresholds');
[ax,h1]=suplabel('lag time from dopamine peak (s)'); 
[ax,h2]=suplabel('Magnitude of the D1 signal','y');
